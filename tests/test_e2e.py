...
class TestAnalyzeURL:
    ...
    def test_facebook_stories_short_circuit_analyse(self, app):
        """
        Facebook Story URLs are short-circuited in analyse_url — no yt-dlp call.
        on_done is called immediately with synthetic MediaInfo (source_engine=facebook_story).
        """
        service = app["service"]
        results = []
        done = threading.Event()
        ...
        service.analyse_url(
            url="https://www.facebook.com/stories/251893823122489/",
            on_done=lambda i: (results.append(i), done.set()),
        ...
        assert results
        assert results[0].source_engine == "facebook_story"
...
class TestDownloadFlow:
    ...
    def test_failed_download_has_error_message(self, app):
        ...
        with patch.object(engine, "download", side_effect=RuntimeError("Video not found")):
            task = service.start_download(
                url=info.url,
                media_info=info,
                format_id="best",
            ...
            assert wait_for(lambda: task.status == DownloadStatus.FAILED), "Task phải chuyển sang FAILED"
...
class TestHistoryAutoSave:
    def test_completed_download_appears_in_history(self, app):
        ...
        assert any(e.get("id") == task.id for e in history_entries), "Task phải xuất hiện trong history"
    ...
    def test_failed_download_also_saved_to_history(self, app):
        ...
        with patch.object(engine, "download", side_effect=RuntimeError("404 Not Found")):
            task = service.start_download(
                url=info.url,
                media_info=info,
                format_id="best",
    ...
    def test_search_history(self, app):
        ...
        with patch.object(engine, "download", side_effect=fake_ydl_download):
            ...
            for info in videos:
                ...
                tasks.append(task)
            assert wait_for(lambda: all(t.status == DownloadStatus.COMPLETED for t in tasks))
            assert wait_for(lambda: len(service.get_history()) >= 2, timeout=3)

        results = service.search_history("rick")
        assert len(results) >= 1
        assert all(
            "rick" in r.get("title", "").lower() or "rick" in r.get("url", "").lower() for r in results
        )
...
class TestEvents:
    ...
    def test_download_completed_event_fired(self, app):
        ...
        bus.subscribe(EventBus.DOWNLOAD_COMPLETED, lambda **kw: events.append("completed"))
    ...
    def test_download_failed_event_fired(self, app):
        ...
        with patch.object(engine, "download", side_effect=RuntimeError("Server error")):
            service.start_download(
                url=info.url,
                media_info=info,
                format_id="best",
...
class TestSecurity:
    ...
    def test_cookie_file_outside_home_rejected(self, app, tmp_path, monkeypatch):
        ...
        assert "cookiefile" not in captured, "cookie_file ngoài config dir không được truyền vào yt-dlp"
