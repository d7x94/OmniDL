...
class TestConvertSyncMp3:
    ...
    def test_mp3_cancelled_before_start(self, tmp_path, fake_ffmpeg, monkeypatch):
        ...
        svc = FfmpegConvertService()
        with pytest.raises(ConversionCancelledError):
            svc._convert_sync(src, "standard", tmp_path, None, cancel_event=cancel, target_ext="mp3")
    ...
    def test_mp3_with_progress_callback(self, tmp_path, fake_ffmpeg, monkeypatch):
        ...
        monkeypatch.setattr(FfmpegConvertService, "_locate_ffmpeg_bin", staticmethod(lambda: fake_ffmpeg))
        monkeypatch.setattr(FfmpegConvertService, "_probe_duration", staticmethod(lambda *a: 10.0))
        monkeypatch.setattr(subprocess, "Popen", fake_popen)
...
class TestConvertSyncRemux:
    def test_remux_to_mkv_returns_mkv_file(self, tmp_path, fake_ffmpeg, monkeypatch):
        ...
        progresses = []
        svc = FfmpegConvertService()
        result = svc._convert_sync(src, "standard", tmp_path, progresses.append, target_ext="mkv")
