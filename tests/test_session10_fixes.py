...
class TestRevealInExplorerResolvesPath(unittest.TestCase):
    ...
    def test_macos_does_not_pass_unresolved_str(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            f = pathlib.Path(d) / "clip.mp4"
            f.write_bytes(b"x")
            raw = str(f)          # e.g. /tmp/xyz/clip.mp4 (might be non-canonical)
            resolved = str(f.resolve())
            args = self._call("darwin", f)
...
class TestOpenFolderResolvesPath(unittest.TestCase):
    ...
    def test_windows_passes_resolved_path(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertEqual(args[1], str(p.resolve()),
                             f"Windows open_folder must use .resolve(); got {args[1]!r}")
    ...
    def test_macos_passes_resolved_path(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertEqual(args[0], "open")
            self.assertEqual(args[1], str(p.resolve()),
                             f"macOS open_folder must use .resolve(); got {args[1]!r}")
    ...
    def test_linux_passes_resolved_path(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertEqual(args[0], "xdg-open")
            self.assertEqual(args[1], str(p.resolve()),
                             f"Linux open_folder must use .resolve(); got {args[1]!r}")
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            for platform in ("win32", "darwin", "linux"):
                with patch("utils.helpers.sys") as ms, \
                    ...
                    open_folder(p)
                    _, kw = mp.call_args
                    self.assertTrue(kw.get("close_fds"),
                                    f"close_fds missing on {platform}")
...
class TestLiveBadgeUsesThemeToken(unittest.TestCase):
    ...
    def test_live_badge_uses_t_error(self):
        """T.error must appear in the _live_badge construction."""
        src_path = _REPO / "ui" / "tabs" / "home_tab.py"
        src = src_path.read_text(encoding="utf-8")
        # Find the _live_badge block
        idx = src.find("self._live_badge = ctk.CTkLabel(")
        self.assertGreater(idx, 0, "_live_badge construction not found")
        snippet = src[idx:idx + 300]
        self.assertIn("T.error", snippet,
                      "live badge must use T.error, not a hardcoded colour")

    def test_on_theme_refreshes_live_badge(self):
        """_on_theme must call configure(fg_color=T.error) on _live_badge."""
        src_path = _REPO / "ui" / "tabs" / "home_tab.py"
        src = src_path.read_text(encoding="utf-8")
        on_theme_idx = src.find("def _on_theme(")
        self.assertGreater(on_theme_idx, 0, "_on_theme not found")
        # Find end of _on_theme (next method def)
        next_def = src.find("\n    def ", on_theme_idx + 1)
        on_theme_body = src[on_theme_idx:next_def]
        self.assertIn("_live_badge", on_theme_body,
                      "_on_theme must refresh _live_badge")
        self.assertIn("T.error", on_theme_body,
                      "_on_theme must use T.error when refreshing live badge")
...
class TestMainWindowThemeRegistration(unittest.TestCase):
    ...
    def test_on_theme_refreshes_sidebar(self):
        ...
        self.assertIn("self._sidebar.configure", snippet,
                      "_on_theme must reconfigure _sidebar")

    def test_on_theme_refreshes_content(self):
        src = (_REPO / "ui" / "main_window.py").read_text(encoding="utf-8")
        idx = src.find("def _on_theme(self)")
        snippet = src[idx:idx + 1500]
        self.assertIn("self._content.configure", snippet,
                      "_on_theme must reconfigure _content")

    def test_on_theme_refreshes_title_bar(self):
        src = (_REPO / "ui" / "main_window.py").read_text(encoding="utf-8")
        idx = src.find("def _on_theme(self)")
        snippet = src[idx:idx + 1500]
        self.assertIn("self._title_bar.configure", snippet,
                      "_on_theme must reconfigure _title_bar")

    def test_on_theme_refreshes_nav_buttons(self):
        src = (_REPO / "ui" / "main_window.py").read_text(encoding="utf-8")
        idx = src.find("def _on_theme(self)")
        snippet = src[idx:idx + 1500]
        self.assertIn("self._nav_btns", snippet,
                      "_on_theme must iterate and reconfigure _nav_btns")

    def test_on_theme_refreshes_section_labels(self):
        src = (_REPO / "ui" / "main_window.py").read_text(encoding="utf-8")
        idx = src.find("def _on_theme(self)")
        snippet = src[idx:idx + 1500]
        self.assertIn("self._section_labels", snippet,
                      "_on_theme must refresh _section_labels")

    def test_toggle_theme_calls_t_set_mode_not_manual_configure(self):
        """
        After the refactor, _toggle_theme must delegate to T.set_mode()
        which fires _on_theme. It must NOT manually configure widgets
        (that's _on_theme's job now).
        """
        src = (_REPO / "ui" / "main_window.py").read_text(encoding="utf-8")
        idx = src.find("def _toggle_theme(self)")
        end = src.find("\n    def ", idx + 1)
        snippet = src[idx:end]
        self.assertIn("T.set_mode(", snippet,
                      "_toggle_theme must call T.set_mode()")

    def test_divider_stored_as_attribute(self):
        """The 1px divider frame must be stored as self._divider for theme refresh."""
        src = (_REPO / "ui" / "main_window.py").read_text(encoding="utf-8")
        self.assertIn("self._divider =", src,
                      "Sidebar divider must be stored as self._divider")

    def test_section_labels_list_exists(self):
        src = (_REPO / "ui" / "main_window.py").read_text(encoding="utf-8")
        self.assertIn("self._section_labels", src,
                      "MainWindow must maintain a self._section_labels list")

    def test_powered_lbl_stored_as_attribute(self):
        src = (_REPO / "ui" / "main_window.py").read_text(encoding="utf-8")
        self.assertIn("self._powered_lbl =", src,
                      "'Powered by yt-dlp' label must be stored for theme refresh")

    def test_wctrl_btns_list_exists(self):
        src = (_REPO / "ui" / "main_window.py").read_text(encoding="utf-8")
        self.assertIn("self._wctrl_btns", src,
                      "Window-control buttons must be collected in self._wctrl_btns")
...
class TestStartupThemeSync(unittest.TestCase):
    ...
    def test_dark_startup_no_palette_change(self):
        ...
        """
        from ui.themes.tokens import T, _DARK
        T.set_mode("dark")
        fired = []
        T.register(lambda: fired.append(1))
        T.set_mode("dark")   # same mode → no-op
        self.assertEqual(fired, [],
                         "No callback should fire when mode is unchanged")
        for key in _DARK:
            self.assertEqual(T.get(key), _DARK[key])

    def test_flicker_prevention_in_main_window(self):
        """
        MainWindow.__init__ must render the window invisible during
        construction so no half-built frame is ever shown.

        Uses attributes('-alpha', 0/1) instead of withdraw/deiconify because
        withdraw() loses the native window handle on Windows when combined
        with overrideredirect(True), causing the window to be destroyed by
        the window manager before _restore_taskbar() can run.
        """
        src = (_REPO / "ui" / "main_window.py").read_text(encoding="utf-8")
        ...
        self.assertIn(
            'attributes("-alpha", 0)', init_body,
            "MainWindow.__init__ must set alpha=0 before building the UI",
        )
        self.assertIn(
            'attributes("-alpha", 1)', init_body,
            "MainWindow.__init__ must restore alpha=1 after building the UI",
        )
        hide_idx = init_body.find('attributes("-alpha", 0)')
        build_idx = init_body.find("self._build_ui()")
        show_idx  = init_body.find('attributes("-alpha", 1)')
        self.assertLess(hide_idx, build_idx, "alpha=0 must precede _build_ui()")
        self.assertGreater(show_idx, build_idx, "alpha=1 must follow _build_ui()")
